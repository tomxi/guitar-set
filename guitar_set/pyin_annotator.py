...
# import guitar_set._pitch_tracker_old as pt
#
#
# def _mono_anal_homebrew(stem_path, open_string_midi):
#     """save jams with same stem_path"""
#     y, sr = librosa.load(stem_path, sr=None)
#     sf, times_on, fs_novelty_on = pt.compute_novelty_spec_flux(y, sr)
#     onset_frames, novelty_smoothed, thresh = pt.onsets_from_novelty(sf,
#                                                                    times_on,
#                                                                  fs_novelty_on)
#     f_detect, big_ann = pt.detect_f0(y, sr, open_string_midi,
...
def transcribe(dirpath):
    ...
    for f in files:
        string_id = basename(f).split('.')[0][0]  # the second char of f
...
def jamses_to_jams(jams_files_dir):
    ...
    for j in jams_files:
        ...
        ann = mono_jams.search(namespace='pitch_midi')[0]
        new_ann = pad_sparse_anno(jams.convert(ann, 'pitch_contour'))
...
def pad_sparse_anno(anno):
    ...
    for note in anno:
        ...
        if pad_dur > 0:
            ts.append(pad_start)
            ds.append(pad_dur)
            vs.append({'voiced': False, 'index': 0, 'frequency': 0})
        pad_start = note.time + note.duration

    pad_dur = anno.duration - pad_start
    if pad_dur > 0:
        ts.append(pad_start)
        ds.append(pad_dur)
        vs.append({'voiced': False, 'index': 0, 'frequency': 0})

    for t, d, v in zip(ts, ds, vs):
        anno.append(time=t, duration=d, value=v)
    return anno
